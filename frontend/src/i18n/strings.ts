...
export const STRINGS = {
  ...
  'status.locate_pc_denied_mac': { zh: 'macOS 定位權限被拒, 請到 系統設定 → 隱私權與安全性 → 定位服務 開啟並允許 LocWarp 存取', en: 'macOS Location access denied. Go to System Settings → Privacy & Security → Location Services, turn it on and allow LocWarp.' },
  ...
  'status.locate_pc_source_wifi_mac': { zh: '來源: macOS 定位 (Wi-Fi / GPS)', en: 'Source: macOS Location (Wi-Fi / GPS)' },
  'status.locate_pc_source_ip': { zh: '來源: IP 定位 (粗略, 通常為城市等級)', en: 'Source: IP geolocation (coarse, usually city-level)' },
  // Locate-source preference (auto / native / ip)
  'status.locate_pc_mode_label': { zh: '定位方式', en: 'Location method' },
  'status.locate_pc_mode_auto': { zh: '自動', en: 'Auto' },
  'status.locate_pc_mode_native': { zh: '原生', en: 'Native' },
  'status.locate_pc_mode_ip': { zh: 'IP', en: 'IP' },
  'status.locate_pc_mode_auto_hint': { zh: '先用系統精確定位, 失敗時自動改用 IP 定位', en: 'Try precise system location first, fall back to IP if it fails' },
  'status.locate_pc_mode_native_hint': { zh: '只用系統精確定位 (需已授權定位權限), 失敗不退回 IP', en: 'System precise location only (needs location permission); no IP fallback' },
  'status.locate_pc_mode_ip_hint': { zh: '直接用 IP 定位 (粗略, 不需定位權限)', en: 'Use IP geolocation directly (coarse, no permission needed)' },
